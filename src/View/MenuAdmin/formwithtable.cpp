    ...
    FormWithTable(identifier, sum, parent)
{
    ...
    supplierModel->updateModelViaQuery(QString("select p_name as 'Назва товару', listS_priceCount as 'Ціна за одиницю (грн.)', p_brand as 'Бренд', p_weight as 'Віга (г.)', p_package as 'Упаковка', p_country as 'Країна', sum(listS_count) as 'Кількість (од.)' from listsupply join waybill using(id_waybill) join product using(id_product) where id_supplier = %1 and waybillDate = '%2' group by id_product;")
